...
fetch(weatherURL)
  ...
  .then((meridianWeather) => {
    ...
    for (i=1; i < 4; i++) {
      ...
      forecast.innerHTML += `<span class="weather-words">${dayofweek[date.getDay()].toUpperCase()}: <br> </span><span class="weather-numbers">${meridianWeather.daily[i].temp.day}&deg; F</span><br>`;
      num += 1;
    }

    //Remember to check that this works on a day with a weather alert

    if (meridianWeather.hasOwnProperty("alerts[0].description") == true) {
